...
def hasintersection(grid, y, x, verbose=True):
    ...
    if x >= gridsize or y >= gridsize or x < 0 or y < 0:
        ...
        if verbose:
            print(f"\tEstá posição já esta ocupada! ({numtocoord(x)}{y})")
        return True
    else:
...
def inputship(grid, shipname):
    ...
    while not placed:
        ...
        while x < 0 or y < 0 or x > gridsize or y > gridsize:
            print(f"Qual coordenada você gostaria de colocar {shipname} ({ships[shipname]} posições) como?\nPor exemplo, B0")
        ...
        if placeship(tempgrid, shipname, y, x, isvertical):
            ...
            if len(cell) != 3:
                ...
                print(f" na {numtocoord(x)}{y}, está certo ? (y/n)")
                choice = input(">: ")
...
def attack(grid, x, y):
    ...
    if grid[y][x] == " ":
        ...
        center("O TIRO CAIU NA AGUÁ!!!!!")
        return 0
    elif grid[y][x].islower() or grid[y][x] == "*":
        return -1
    else:
        grid[y][x] = grid[y][x].lower()
        center("KABOOOOOOOM!!!!!.VOCÊ ACERTOU UMA EMBARCAÇÃO.")
        return 1
...
while True:
    ...
    ### number of players
    print("Escolha o número de jogadores de 0 a 2:")
    while True:
        playercount = input(">: ")
        try:
    ...
    if playercount > 0:
        while True:
            p1auto = input(f"{p1name}, Gostaria de começar com um tabuleiro aleatório? (y/n)\n>: ")
            if p1auto == "y":
                p1auto = True
                break
            elif p1auto == "n":
    ...
    if playercount == 2:
        while True:
            p2auto = input(f"{p2name}, Gostaria de começar com um tabuleiro aleatório? (y/n)\n>: ")
            if p2auto == "y":
                p2auto = True
                break
            elif p2auto == "n":
    ...
    while True:
        ...
        if playercount == 2:
            ...
            #printgrid(gridp2) #Essa linha exibe a posição dos barcos inimigos
